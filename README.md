...
- The DMG is currently built for Apple Silicon Macs running macOS 13 or later
- The build is unsigned, so the first launch may require `Control` + click -> `Open` or `Open Anyway` in System Settings
...
│   ├── build_macos_app.sh
│   ├── postprocess_distribution_bundle.py
│   └── verify_distribution_bundle.py
├── tests/
├── desktop/
│   ├── __main__.py
│   ├── app.py
...
If you do not want to build locally, use the prebuilt `.dmg` from GitHub Releases instead. The DMG is the official Apple Silicon distributable; the repo checkout launcher is only for local development/use on this source tree.
...
- `dist/md2pdf-converter.app`: official unsigned Apple Silicon distributable bundle generated by `py2app`
- `dist/md2pdf-converter-unsigned.dmg`: DMG that wraps the same distributable `dist/md2pdf-converter.app`

The build now also:

- post-processes the bundle so WeasyPrint can find its packaged native libraries without Homebrew
- copies bundled fontconfig resources for the distributable app
- verifies the bundle has no `/opt/homebrew` or `/usr/local/lib` runtime dependencies
- runs a bundled smoke test before producing the DMG
...
Because the build is unsigned, macOS may block the first launch. Open it with `Control` + click -> `Open`, or use `Open Anyway` in System Settings after the first launch attempt.
...
- The packaged desktop app is currently targeted at Apple Silicon Macs running macOS 13 or later and unsigned local sharing.
- The root `md2pdf-converter.app` is a local launcher wrapper, while the official distributable unsigned bundle is `dist/md2pdf-converter.app`.
- If the distributable app opens but Markdown-to-PDF conversion fails, check the log at `~/Library/Application Support/md2pdf-converter/logs/app.log`.
